...
const AdminWishlists: React.FC = () => {
	...
	const deleteWishlistItem = async (wishlistId: string) => {
		if (
			!confirm('Are you sure you want to remove this item from the wishlist?')
		) {
			return;
		}
	...
	const clearUserWishlist = async (userId: string, userName: string) => {
		if (
			!confirm(
				`Are you sure you want to clear all wishlist items for ${userName}?`
			)
		) {
			return;
		}
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : 'Failed to clear user wishlist'
			);
		}
	};
				...
				{totalPages > 1 && (
					<div className="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
						<div className="text-sm text-gray-500">
							...
							{Math.min(currentPage * itemsPerPage, totalWishlists)} of{' '}
							{totalWishlists} wishlist items
						</div>
