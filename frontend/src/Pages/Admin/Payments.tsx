...
export default function AdminPayments() {
	...
	const convertPaymentToFormData = (
	...
	const handleSubmit = async (formData: PaymentMethodFormData) => {
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to ${editingPayment ? 'update' : 'create'} payment method`
				);
			}
			...
			console.error(
				`Error ${editingPayment ? 'updating' : 'creating'} payment method:`,
				err
			);
			setError(
	...
	return (
		<div className="space-y-6">
							...
							{loading ? (
								...
								payments.map((payment) => (
									<tr key={payment.id} className="hover:bg-gray-50">
										...
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="text-sm text-gray-900">{payment.provider}</div>
										</td>
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="text-sm text-gray-900">****{payment.last4}</div>
											<div className="text-sm text-gray-500">
												Exp: {formatExpiryDate(payment.expiryMonth, payment.expiryYear)}
											</div>
										</td>
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="text-sm text-gray-900">{payment.holderName}</div>
										</td>
										<td className="px-6 py-4 whitespace-nowrap">
			...
			<FormModal<PaymentMethodFormData>
				...
				initialData={
					editingPayment
						? convertPaymentToFormData(editingPayment)
						: {
								userId: '',
								type: 'credit_card',
								provider: '',
