...
const AdminCategories: React.FC = () => {
	...
	const [viewingCategory, setViewingCategory] =
		useState<CategoryViewData | null>(null);
	...
	const fetchCategories = async (page: number = 1, search: string = '') => {
		try {
			...
			setParentCategories(
				availableParents.map((cat) => ({ id: cat.id, name: cat.name }))
			);
		} catch (err) {
	...
	const handleSubmitCategory = async (formData: CategoryFormData) => {
		try {
			...
			setParentCategories((prev) => [
				...prev,
				{ id: editingCategory?.id || '', name: formData.name as string },
			]);
	...
	return (
		<div className="space-y-6">
			...
			<FormModal<CategoryFormData>
				...
				initialData={
					editingCategory ? convertCategoryToFormData(editingCategory) : undefined
				}
